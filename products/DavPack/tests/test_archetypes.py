...
##############################################################################
"""
$Id: test_archetypes.py 2331 2008-09-26 21:12:06Z sidnei $
"""
...
prefix = package_home(globals())

ZopeTestCase.installProduct('Archetypes')
ZopeTestCase.installProduct('DavPack')

from Products.Archetypes.BaseContent import BaseContent
from Products.Archetypes.public import MetadataStorage
from Products.DavPack.at.utils import _isUnwrappedWrappable
...
class FileFieldTest(ZopeTestCase.ZopeTestCase):
    ...
    def test_factory(self):
        from OFS.Image import File
        self.failUnless(self.factory is File)
    ...
    def test_string_pdf(self):
        f = open(os.path.join(prefix, 'data', 'webdav.pdf')).read()
        v, m, f = self.field._process_input(f, instance=self.instance)
        self.failUnless(isinstance(v, self.factory), (type(v), self.factory))
        self.assertEquals(m, 'application/pdf')
        self.failIf(f)
    ...
    def test_isUnwrappedWrappable(self):
        # simple types should never be unwrapped wrappables
        for obj in [None,
                    1,
                    True,
                    False,
                    ...
                    ("a", "tuple"),
                    ["a", "list"] ]:
            self.failIf(_isUnwrappedWrappable(obj))
        # regular Acquisition derived object, just created
        # is an unwrapped wrappable
        f = File('f', '', '')
        self.failUnless(_isUnwrappedWrappable(f))
        # now if this same object is wrapped in some other one
        # then it obviously isn't unwrapped anymore
        wf = f.__of__(self.folder)
        self.failIf(_isUnwrappedWrappable(wf))
    ...
    def test_get(self):
        ...
        self.failIf(_isUnwrappedWrappable(result))
...
class TextFieldTest(FileFieldTest):
    ...
    def test_factory(self):
        ...
        self.failIf(self.factory is File)
    ...
    def test_get(self):
        ...
        # For TextField, we should really return a string for
        # backwards compatibility.
        self.failUnless(isinstance(result, basestring), type(result))
        self.failIf(_isUnwrappedWrappable(result))
    ...
    def test1_get_baseunit_ofs_file_text_field_migrate(self):
        from Products.Archetypes.BaseUnit import BaseUnit
        f = File(self.field.getName(), '', '')
        self.folder._setObject(self.field.getName(), f)
        result = self.field.getBaseUnit(instance=self.folder)
        self.failUnless(isinstance(result, BaseUnit), type(result))
        value = self.folder[self.field.getName()]
        self.failUnless(isinstance(value, BaseUnit), type(value))
...
def test_suite():
    ...
    suite.addTest(
        doctest.FunctionalDocFileSuite('archetypes-events.txt',
